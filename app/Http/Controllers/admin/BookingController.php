            ...
            try {
                if ($request->has('manage_slot') && $request->manage_slot == 1) {
                    ...
                    try {
                        ...
                        $data = ['title' => 'Booking Extend Time', 'email' => $bookingdata->customer_email, 'logo' => Helper::image_path('logo.png'), 'booking_id' => $bookingdata->booking_id, 'booki0ng_date' => $bookingdata->start_date, 'time' => $slot, '', 'payment_link' => URL::to('/payment/' . $bookingdata->token), 'sub_total' => Helper::currency_format($slot_price), 'service_fee' => Helper::currency_format($service_fee), 'hst' => Helper::currency_format($hst), 'total_amount' => Helper::currency_format($total_amount)];
