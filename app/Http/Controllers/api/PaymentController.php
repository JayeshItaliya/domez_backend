...
class PaymentController extends Controller
{
    ...
    public function payment(Request $request)
    {
        ...
        try {
            ...
            if ($booking->type == 1) {
                $data = ['title' => 'Booking Confirmation Receipt', 'type' => $booking->type, 'email' => $booking->customer_email, 'name' => $booking->customer_name ?? '', 'booking_id' => $booking->booking_id, 'booking_date' => $booking->start_date, 'start_time' => $booking->start_time, 'end_time' => $booking->end_time, 'dome_name' => $booking->dome_info->dome_name, 'dome_address' => $booking->dome_info->dome_address, 'field_name' => $booking->field_info->name, 'players' => $booking->players, 'paid_amount' => $booking->paid_amount, 'due_amount' => $booking->due_amount, 'total_amount' => $booking->total_amount];
            } else {
                $data = ['title' => 'Booking Confirmation Receipt', 'type' => $booking->type, 'email' => $booking->customer_email, 'name' => $booking->customer_name ?? '', 'booking_id' => $booking->booking_id, 'start_date' => $booking->start_date, 'end_date' => $booking->end_date, 'start_time' => $booking->start_time, 'end_time' => $booking->end_time, 'league_name' => $booking->league_info->name, 'dome_name' => $booking->dome_info->dome_name, 'dome_address' => $booking->dome_info->dome_address, 'field_name' => $booking->field_info->name, 'players' => $booking->players, 'paid_amount' => $booking->paid_amount, 'due_amount' => $booking->due_amount, 'total_amount' => $booking->total_amount];
            }
