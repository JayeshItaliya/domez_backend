    ...
    public function booking_details(Request $request)
    {
        ...
        $gettransaction = Transaction::leftJoin('users AS user', function ($query) {
            ...
            ->where('transactions.booking_id', $booking->booking_id)->select(
                ...
                DB::raw("CAST(transactions.user_id AS UNSIGNED) as user_id"),
                DB::raw("CAST(transactions.amount AS DOUBLE) as amount"),
        ...
        $booking_details = [
            ...
            "booking_status_original" => $booking->booking_status,
            "payment_type" => $booking->payment_type,
            "min_split_amount" => $booking->min_split_amount,

            "booking_created_at" => Carbon::parse($booking->booking_mode == 1 ? $booking->created_at : ($booking->booking_accepted_at != "" ? $booking->booking_accepted_at :$booking->created_at) )->setTimezone(env('SET_TIMEZONE'))->toDateTimeString(),

            "booking_mode" => $booking->booking_mode,
            "age_discount_amount" => $booking->age_discount_amount,
            "fields_discount_amount" => $booking->fields_discount_amount,
        ];
        ...
        try {
            ...
            if ($refund == 1) {
                ...
                foreach (explode(',', $checkbooking->slots) as $key => $slot) {
                    SetPricesDaysSlots::where('start_time', date('H:i', strtotime(explode(' - ', $slot)[0])))->where('end_time', date('H:i', strtotime(explode(' - ', $slot)[1])))->where('sport_id', $checkbooking->sport_id)->whereDate('date', date('Y-m-d', strtotime($checkbooking->start_date)))->update(['status' => 1]);
                }
                ...
                return response()->json(['status' => 0, 'message' => 'Unable to refund amount!'], 200);
            }
        } catch (\Throwable $th) {
